	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
	     ...
	     try{
	    	 ...
	    	 con = (Connection) DriverManager.getConnection(
	    			 "jdbc:mysql://127.0.0.1:3306/users","root","1120..");
	    	 ...
	    	 ResultSet rs =(ResultSet) sta
	    			 .executeQuery("select* from user where user='" + user 
	    			 		+ "' and pwd='" + pwd + "'");
	    	 if(rs.next()){
	    		 String  ll  = rs.getString("user");
	    		 String  mm  = rs.getString("pwd");
	    		    if (ll.equals(user) && mm.equals(pwd)){
	    		    	HttpSession session = request.getSession();
	    		    	session.setAttribute("username", user);
	    		    	session.setAttribute("pwd", pwd);
	    		    	session.setMaxInactiveInterval(10);
	    		    	System.out.println("你的信息已存入");
	    		    	request.getRequestDispatcher("house").forward(request, response);
	    		    }
	    	 }else{
